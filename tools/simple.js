 ...
 * @param {object} sock - The context object.
 * @param {string} id - The ID of the user.
 * @returns {number} Returns 1 if the user is an admin, otherwise returns 0.
 */
async function checkAdmin(sock, id) {
    const group = await sock._client.groupMetadata(sock.id);
 ...
 */
exports.isCmd = (m, sock) => {
    const prefixRegex = new RegExp(sock._config.prefix, 'i');
    ...
    for (const cmd of sock._config.cmd.values()) {
 ...
 * @returns {number} Returns 1 if the user is an admin, otherwise returns 0.
 */
exports.isAdmin = async (sock, number) => {
    const isAdmin = await checkAdmin(sock, number || sock._sender.jid.split('@')[0]);
    return isAdmin ? 1 : 0;
}
 ...
 */
exports.isAdminOf = async (sock) => {
    const isAdminOfGroup = await checkAdmin(sock, sock._client.user.id.split(':')[0]);
